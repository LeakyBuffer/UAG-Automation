...
#*v12.1 - Fixed an issue accidentally deleting the master template INI file
#*v12.2 - Added capability for script to use different uagdeploy.ps1 script version by specifying the folder path, put the uagdeploy.ps1 & .psm in seperate folders and specify full folder path in master-uag-list.csv column for 'uagdeployPath'
#*v12.3 - Added fix for VC inventory name that an issue was found with
#*v13 - Added session statistics menu option under day 2 ops to provide all session stats on 60 second loop for a given Horizon Pod as a way to monitor if no other tooling is available such as V4H
#*v13.1 - Modified source data to not be appended with local working directory of script so full path to OVA needs to be provided in CSV but allows overall more flexibilty.
#*V13.2 - Added the following RADIUS based settings to the master CSV and these attributes will need to be in the template INI in CASE-SENSATIVE FORM! So be sure they are properly spelled in template INI with case sesnsitivity: "hostName","authType","authPort","radiusDisplayHint","accountingPort","hostName_2","authPort_2","accountingPort_2","numAttempts","numAttempts_2","realmPrefix","realmPrefix_2","realmSuffix","realmSuffix_2","serverTimeout","serverTimeout_2","matchWindowsUserName","radiusClassAttributeList","windowsSSOEnabled"
#*V13.2 - Added the following SAML based settings to the master CSV and these attributes also need to be in the template INI in CASE-SENSATIVE form: "authMethods","idpEntityID","metadataXmlFile","forceAuthN"
#*v13.3 - Fixed issue with setting root passwords on UAG in day 2 ops menu as it was a variable issue within the foreach loop that arose when chaning more than 1 UAG root password
#*v13.4 - Added debug column to CSV to allow for not cleaning up and deleting the OVFTool log file, and more importantly the temporary UAG INI used before deploying.
#
#Notes
#--------
...
#Successfully tested deployment with 3.9.1, 3.10, 2009, 2012 and 2103
#
#Asks
#--------
#None
...
while ($ScriptRun) {
    ...
    if ($MenuAction -eq "1") { #MENU-DEPLOY
                ...
                if ($VMList | Where-Object {$_.Name -eq $CreateVM.UAGname}) {
                    ...
                    if ($UAGiniTemplate) {
                        ...
                        foreach ($iniLine in $UAGiniTemplate) {
                            ...
                            } elseif ($iniLine -imatch "name=" -and $iniLine -inotmatch $("name=" + $CreateVM.UAGmonitoringUsername)) {
                                $UAGini += "name=" + $CreateVM.VCname
                                write-host "MODIFY: name="$CreateVM.VCname
                            } elseif ($iniLine -match "source=") {
                                $UAGini += "source=" + $CreateVM.source
                                write-host "MODIFY: source="$CreateVM.source
                            ...
                            } elseif($iniLine -imatch "hostName=" -and $CreateVM.hostName) {
                                #See if RADIUS hostName is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "hostName=" + $CreateVM.hostName
                                write-host "MODIFY: hostName="$CreateVM.hostName
                            } elseif($iniLine -imatch "authType=" -and $CreateVM.authType) {
                                #See if RADIUS authType is present in template INI and if so set it to the value contained in the CSV (PAP, CHAP, MSCHAP1, MSCHAP2)
                                $UAGini += "authType=" + $CreateVM.authType
                                write-host "MODIFY: authType="$CreateVM.authType
                            } elseif($iniLine -imatch "authPort=" -and $CreateVM.authPort) {
                                #See if RADIUS authPort is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "authPort=" + $CreateVM.authPort
                                write-host "MODIFY: authPort="$CreateVM.authPort
                            } elseif($iniLine -imatch "radiusDisplayHint=" -and $CreateVM.radiusDisplayHint) {
                                #See if RADIUS radiusDisplayHint is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "radiusDisplayHint=" + $CreateVM.radiusDisplayHint
                                write-host "MODIFY: radiusDisplayHint="$CreateVM.radiusDisplayHint
                            } elseif($iniLine -imatch "accountingPort=" -and $CreateVM.accountingPort) {
                                #See if RADIUS accountingPort is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "accountingPort=" + $CreateVM.accountingPort
                                write-host "MODIFY: accountingPort="$CreateVM.accountingPort
                            } elseif($iniLine -imatch "hostName_2=" -and $CreateVM.hostName_2) {
                                #See if RADIUS hostName_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "hostName_2=" + $CreateVM.hostName_2
                                write-host "MODIFY: hostName_2="$CreateVM.hostName_2
                            } elseif($iniLine -imatch "authPort_2=" -and $CreateVM.authPort_2) {
                                #See if RADIUS authPort_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "authPort_2=" + $CreateVM.authPort_2
                                write-host "MODIFY: authPort_2="$CreateVM.authPort_2
                            } elseif($iniLine -imatch "accountingPort_2=" -and $CreateVM.accountingPort_2) {
                                #See if RADIUS accountingPort_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "accountingPort_2=" + $CreateVM.accountingPort_2
                                write-host "MODIFY: accountingPort_2="$CreateVM.accountingPort_2
                            } elseif($iniLine -imatch "authMethods=" -and $CreateVM.authMethods) {
                                #See if RADIUS authMethods is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "authMethods=" + $CreateVM.authMethods
                                write-host "MODIFY: authMethods="$CreateVM.authMethods
                            } elseif($iniLine -imatch "idpEntityID=" -and $CreateVM.idpEntityID) {
                                #See if RADIUS idpEntityID is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "idpEntityID=" + $CreateVM.idpEntityID
                                write-host "MODIFY: idpEntityID="$CreateVM.idpEntityID
                            } elseif($iniLine -imatch "metadataXmlFile=" -and $CreateVM.metadataXmlFile) {
                                #See if RADIUS metadataXmlFile is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "metadataXmlFile=" + $CreateVM.metadataXmlFile
                                write-host "MODIFY: metadataXmlFile="$CreateVM.metadataXmlFile
                            } elseif($iniLine -imatch "forceAuthN=" -and $CreateVM.forceAuthN) {
                                #See if RADIUS forceAuthN is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "forceAuthN=" + $CreateVM.forceAuthN
                                write-host "MODIFY: forceAuthN="$CreateVM.forceAuthN
                            } elseif($iniLine -imatch "numAttempts=" -and $CreateVM.numAttempts) {
                                #See if RADIUS numAttempts is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "numAttempts=" + $CreateVM.numAttempts
                                write-host "MODIFY: numAttempts="$CreateVM.numAttempts
                            } elseif($iniLine -imatch "numAttempts_2=" -and $CreateVM.numAttempts_2) {
                                #See if RADIUS numAttempts_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "numAttempts_2=" + $CreateVM.numAttempts_2
                                write-host "MODIFY: numAttempts_2="$CreateVM.numAttempts_2
                            } elseif($iniLine -imatch "realmPrefix=" -and $CreateVM.realmPrefix) {
                                #See if RADIUS realmPrefix is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "realmPrefix=" + $CreateVM.realmPrefix
                                write-host "MODIFY: realmPrefix="$CreateVM.realmPrefix
                            } elseif($iniLine -imatch "realmPrefix_2=" -and $CreateVM.realmPrefix_2) {
                                #See if RADIUS realmPrefix_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "realmPrefix_2=" + $CreateVM.realmPrefix_2
                                write-host "MODIFY: realmPrefix_2="$CreateVM.realmPrefix_2
                            } elseif($iniLine -imatch "realmSuffix=" -and $CreateVM.realmSuffix) {
                                #See if RADIUS realmSuffix is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "realmSuffix=" + $CreateVM.realmSuffix
                                write-host "MODIFY: realmSuffix="$CreateVM.realmSuffix
                            } elseif($iniLine -imatch "realmSuffix_2=" -and $CreateVM.realmSuffix_2) {
                                #See if RADIUS realmSuffix_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "realmSuffix_2=" + $CreateVM.realmSuffix_2
                                write-host "MODIFY: realmSuffix_2="$CreateVM.realmSuffix_2
                            } elseif($iniLine -imatch "serverTimeout=" -and $CreateVM.serverTimeout) {
                                #See if RADIUS serverTimeout is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "serverTimeout=" + $CreateVM.serverTimeout
                                write-host "MODIFY: serverTimeout="$CreateVM.serverTimeout
                            } elseif($iniLine -imatch "serverTimeout_2=" -and $CreateVM.serverTimeout_2) {
                                #See if RADIUS serverTimeout_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "serverTimeout_2=" + $CreateVM.serverTimeout_2
                                write-host "MODIFY: serverTimeout_2="$CreateVM.serverTimeout_2
                            } elseif($iniLine -imatch "matchWindowsUserName=" -and $CreateVM.matchWindowsUserName) {
                                #See if RADIUS matchWindowsUserName is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "matchWindowsUserName=" + $CreateVM.matchWindowsUserName
                                write-host "MODIFY: matchWindowsUserName="$CreateVM.matchWindowsUserName
                            } elseif($iniLine -imatch "radiusClassAttributeList=" -and $CreateVM.radiusClassAttributeList) {
                                #See if RADIUS radiusClassAttributeList is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "radiusClassAttributeList=" + $CreateVM.radiusClassAttributeList
                                write-host "MODIFY: radiusClassAttributeList="$CreateVM.radiusClassAttributeList
                            } elseif($iniLine -imatch "windowsSSOEnabled=" -and $CreateVM.windowsSSOEnabled) {
                                #See if RADIUS windowsSSOEnabled is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "windowsSSOEnabled=" + $CreateVM.windowsSSOEnabled
                                write-host "MODIFY: windowsSSOEnabled="$CreateVM.windowsSSOEnabled
                            } else {
                                $UAGini += $iniLine
                            }
                        }
                        ...
                        if ($RoleMonitoringUserPresent -and ($CreateVM.source -inotmatch "3.9.1")) {
                            if ($uagMonitoringkey) {
                                ...
                                $cmd = @()
                                $cmd = $CreateVM.uagdeployPath + "\uagdeploy.ps1" + " -iniFile `".\TempUAGini.ini`"" + " -rootPwd " + $UAGrootPWunsec + " -adminPwd " + $UAGadminPWunsec + " -newAdminUserPwd " + $monitoringcred + " -ceipEnabled " + $false + " -noSSLVerify " + $true
                                ...
                                $cmd = @()
                                $cmd = $CreateVM.uagdeployPath + "\uagdeploy.ps1" + " -iniFile `".\TempUAGini.ini`"" + " -rootPwd " + $UAGrootPWunsec + " -adminPwd " + $UAGadminPWunsec + " -ceipEnabled " + $false + " -noSSLVerify " + $true
                                Invoke-Expression $cmd
                                $cmd = @() 
                            } 
                        } else {
                    ...
                    if ($UAGiniTemplate) {
                        ...
                        foreach ($iniLine in $UAGiniTemplate) {
                            if ($iniLine -match "defaultGateway=") { 
                                ...
                                $UAGini += "source=" + $UAGvm.source
                                write-host "MODIFY: source="$UAGvm.source
                            } elseif ($iniLine -match "ds=") {
                                $UAGini += "ds=" + $UAGvm.ds
                            ...
                            } elseif($iniLine -imatch "hostName=" -and $UAGvm.hostName) {
                                #See if RADIUS hostName is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "hostName=" + $UAGvm.hostName
                                write-host "MODIFY: hostName="$UAGvm.hostName
                            } elseif($iniLine -imatch "authType=" -and $UAGvm.authType) {
                                #See if RADIUS authType is present in template INI and if so set it to the value contained in the CSV (PAP, CHAP, MSCHAP1, MSCHAP2)
                                $UAGini += "authType=" + $UAGvm.authType
                                write-host "MODIFY: authType="$UAGvm.authType
                            } elseif($iniLine -imatch "authPort=" -and $UAGvm.authPort) {
                                #See if RADIUS authPort is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "authPort=" + $UAGvm.authPort
                                write-host "MODIFY: authPort="$UAGvm.authPort
                            } elseif($iniLine -imatch "radiusDisplayHint=" -and $UAGvm.radiusDisplayHint) {
                                #See if RADIUS radiusDisplayHint is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "radiusDisplayHint=" + $UAGvm.radiusDisplayHint
                                write-host "MODIFY: radiusDisplayHint="$UAGvm.radiusDisplayHint
                            } elseif($iniLine -imatch "accountingPort=" -and $UAGvm.accountingPort) {
                                #See if RADIUS accountingPort is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "accountingPort=" + $UAGvm.accountingPort
                                write-host "MODIFY: accountingPort="$UAGvm.accountingPort
                            } elseif($iniLine -imatch "hostName_2=" -and $UAGvm.hostName_2) {
                                #See if RADIUS hostName_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "hostName_2=" + $UAGvm.hostName_2
                                write-host "MODIFY: hostName_2="$UAGvm.hostName_2
                            } elseif($iniLine -imatch "authPort_2=" -and $UAGvm.authPort_2) {
                                #See if RADIUS authPort_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "authPort_2=" + $UAGvm.authPort_2
                                write-host "MODIFY: authPort_2="$UAGvm.authPort_2
                            } elseif($iniLine -imatch "accountingPort_2=" -and $UAGvm.accountingPort_2) {
                                #See if RADIUS accountingPort_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "accountingPort_2=" + $UAGvm.accountingPort_2
                                write-host "MODIFY: accountingPort_2="$UAGvm.accountingPort_2
                            } elseif($iniLine -imatch "authMethods=" -and $UAGvm.authMethods) {
                                #See if RADIUS authMethods is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "authMethods=" + $UAGvm.authMethods
                                write-host "MODIFY: authMethods="$UAGvm.authMethods
                            } elseif($iniLine -imatch "idpEntityID=" -and $UAGvm.idpEntityID) {
                                #See if RADIUS idpEntityID is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "idpEntityID=" + $UAGvm.idpEntityID
                                write-host "MODIFY: idpEntityID="$UAGvm.idpEntityID
                            } elseif($iniLine -imatch "metadataXmlFile=" -and $UAGvm.metadataXmlFile) {
                                #See if RADIUS metadataXmlFile is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "metadataXmlFile=" + $UAGvm.metadataXmlFile
                                write-host "MODIFY: metadataXmlFile="$UAGvm.metadataXmlFile
                            } elseif($iniLine -imatch "forceAuthN=" -and $UAGvm.forceAuthN) {
                                #See if RADIUS forceAuthN is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "forceAuthN=" + $UAGvm.forceAuthN
                                write-host "MODIFY: forceAuthN="$UAGvm.forceAuthN
                            } elseif($iniLine -imatch "numAttempts=" -and $UAGvm.numAttempts) {
                                #See if RADIUS numAttempts is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "numAttempts=" + $UAGvm.numAttempts
                                write-host "MODIFY: numAttempts="$UAGvm.numAttempts
                            } elseif($iniLine -imatch "numAttempts_2=" -and $UAGvm.numAttempts_2) {
                                #See if RADIUS numAttempts_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "numAttempts_2=" + $UAGvm.numAttempts_2
                                write-host "MODIFY: numAttempts_2="$UAGvm.numAttempts_2
                            } elseif($iniLine -imatch "realmPrefix=" -and $UAGvm.realmPrefix) {
                                #See if RADIUS realmPrefix is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "realmPrefix=" + $UAGvm.realmPrefix
                                write-host "MODIFY: realmPrefix="$UAGvm.realmPrefix
                            } elseif($iniLine -imatch "realmPrefix_2=" -and $UAGvm.realmPrefix_2) {
                                #See if RADIUS realmPrefix_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "realmPrefix_2=" + $UAGvm.realmPrefix_2
                                write-host "MODIFY: realmPrefix_2="$UAGvm.realmPrefix_2
                            } elseif($iniLine -imatch "realmSuffix=" -and $UAGvm.realmSuffix) {
                                #See if RADIUS realmSuffix is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "realmSuffix=" + $UAGvm.realmSuffix
                                write-host "MODIFY: realmSuffix="$UAGvm.realmSuffix
                            } elseif($iniLine -imatch "realmSuffix_2=" -and $UAGvm.realmSuffix_2) {
                                #See if RADIUS realmSuffix_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "realmSuffix_2=" + $UAGvm.realmSuffix_2
                                write-host "MODIFY: realmSuffix_2="$UAGvm.realmSuffix_2
                            } elseif($iniLine -imatch "serverTimeout=" -and $UAGvm.serverTimeout) {
                                #See if RADIUS serverTimeout is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "serverTimeout=" + $UAGvm.serverTimeout
                                write-host "MODIFY: serverTimeout="$UAGvm.serverTimeout
                            } elseif($iniLine -imatch "serverTimeout_2=" -and $UAGvm.serverTimeout_2) {
                                #See if RADIUS serverTimeout_2 is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "serverTimeout_2=" + $UAGvm.serverTimeout_2
                                write-host "MODIFY: serverTimeout_2="$UAGvm.serverTimeout_2
                            } elseif($iniLine -imatch "matchWindowsUserName=" -and $UAGvm.matchWindowsUserName) {
                                #See if RADIUS matchWindowsUserName is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "matchWindowsUserName=" + $UAGvm.matchWindowsUserName
                                write-host "MODIFY: matchWindowsUserName="$UAGvm.matchWindowsUserName
                            } elseif($iniLine -imatch "radiusClassAttributeList=" -and $UAGvm.radiusClassAttributeList) {
                                #See if RADIUS radiusClassAttributeList is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "radiusClassAttributeList=" + $UAGvm.radiusClassAttributeList
                                write-host "MODIFY: radiusClassAttributeList="$UAGvm.radiusClassAttributeList
                            } elseif($iniLine -imatch "windowsSSOEnabled=" -and $UAGvm.windowsSSOEnabled) {
                                #See if RADIUS windowsSSOEnabled is present in template INI and if so set it to the value contained in the CSV
                                $UAGini += "windowsSSOEnabled=" + $UAGvm.windowsSSOEnabled
                                write-host "MODIFY: windowsSSOEnabled="$UAGvm.windowsSSOEnabled
                            } else {
                                $UAGini += $iniLine
                            }
                        }
                        ...
                        if ($RoleMonitoringUserPresent -and ($UAGvm.source -inotmatch "3.9.1")) {
                            if ($uagMonitoringkey) {
                                ...
                                #.\uagdeploy.ps1 -iniFile ".\TempUAGini.ini" -rootPwd $UAGrootPWunsec -adminPwd $UAGadminPWunsec -newAdminUserPwd $monitoringcred -ceipEnabled $false -noSSLVerify $true
                                $cmd = @()
                                $cmd = $UAGvm.uagdeployPath + "\uagdeploy.ps1" + " -iniFile `".\TempUAGini.ini`"" + " -rootPwd " + $UAGrootPWunsec + " -adminPwd " + $UAGadminPWunsec + " -newAdminUserPwd " + $monitoringcred + " -ceipEnabled " + $false + " -noSSLVerify " + $true
                                Invoke-Expression $cmd
                                $cmd = @()                                   
                                $RoleMonitoringUserPresent = $false
                            } else {
                                ...
                                #.\uagdeploy.ps1 -iniFile ".\TempUAGini.ini" -rootPwd $UAGrootPWunsec -adminPwd $UAGadminPWunsec -ceipEnabled $false -noSSLVerify $true
                                $cmd = @()
                                $cmd = $UAGvm.uagdeployPath + "\uagdeploy.ps1" + " -iniFile `".\TempUAGini.ini`"" + " -rootPwd " + $UAGrootPWunsec + " -adminPwd " + $UAGadminPWunsec + " -ceipEnabled " + $false + " -noSSLVerify " + $true
                                Invoke-Expression $cmd
                                $cmd = @()
                            } 
                        } else {
        ...
        while ($UpdateMode) {
            ...
            $UpdateOption = Read-Host "[1] - Update Root passwords, [2] - Update Admin passwords, [3] - Update Monitoring passwords, [4] - Quiesce mode, [5] - Certificates, [6] Session Stats or [7] - Done"
...
"@
            ...
            } elseif ($UpdateOption -eq "6") { #MENU-DAY2OPS-SESSIONSTATS
                #Session Stats mode selection
                $HorizonPod = Read-Host -Prompt "Enter the name (Case Sensitive) of the Horizon Pod for which to update all UAG's configured in CSV for the 'PodName' column?"
                $CSV = Import-Csv -Path ".\master-uag-list.csv"
                if (!$CSV) {
                    write-host "FATAL: Could not load 'master-uage-list.csv', please ensure it is present in the same directory as the script!"
                ...
                $SessionStatsModeUAGs = $CSV | Where-Object {$_.PodName -imatch $HorizonPod}
                if ($SessionStatsModeUAGs) { 
                    $SessionStatsMode = $true
                    #Embed the user in a while loop to allow for modifications at the Horizon Pod level for day 2 ops
                    while ($SessionStatsMode) { #SESSIONSTATS MODE MENU LOOP
                        $SessionStatsOption = Read-Host "[1] - Display Session Stats, [2] Done for ($HorizonPod)"
                        if ($SessionStatsOption -eq "1") { #MENU-DAY2OPS-SESSIONSTATS-DISPLAY
                            #Get UAG Session Stats
add-type @"
    using System.Net;
    using System.Security.Cryptography.X509Certificates;
    public class TrustAllCertsPolicy : ICertificatePolicy {
...
"@
                            [System.Net.ServicePointManager]::CertificatePolicy = New-Object TrustAllCertsPolicy
                            $loop = $true
                            write-host "WARNING! - PRESS '1' TO STOP THE SESSION STATS DISPLAY AS IT WILL CONTINUOUSLY LOOP"
                            while ($loop) {
                                foreach ($SessionStatsVM in $SessionStatsModeUAGs) {
                                    $key = @()
                                    $SecurePassword = @()
                                    $BSTR = @()
                                    $UserUnsecurePassword = @()
                                    ...
                                    $Userkeyfoo = @()
                                    $UserSessionStatfoo = @()
                                    $GetSessionStatsUserUrl = @()
                                    $SessionStat = @()
                                    $GetSessionStatsUserUrl = "https://" + $SessionStatsVM.ip0 + ":9443/rest/v1/monitor/stats"
                                    if ($SessionStatsVM.Skipped -eq 1) {
                                        #VM record is set to be disabled so skip this process
                                        #write-host "WARNING: Record for this object is set Skipped = 1, so skipping..."
                                        continue
                                    } else {}
                                    $key = Get-Content $SessionStatsVM.UAGadminkey
                                    if (!$key) {
                                        write-host "FATAL: Could not load" $SessionStatsVM.UAGadminkey ", please ensure it is present in the same directory as the script!"
                                        Stop-Transcript
                                        exit
                                    } else {}
                                    $SecurePassword = ($SessionStatsVM.UAGadminPW | ConvertTo-SecureString -Key $key)
                                    $BSTR = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($SecurePassword)
                                    $UserUnsecurePassword = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($BSTR)
                                    [Runtime.InteropServices.Marshal]::ZeroFreeBSTR($BSTR)
                                    $UserName = "admin"
                                    ...
                                    #Try to run the invoke-rest method to get the certificates for end_user and admin
                                    try {
                                        $SessionStat = Invoke-RestMethod -Uri $GetSessionStatsUserUrl -Method Get -ContentType "application/json" -Credential $mycreds
                                        #UAG SESSION STAT DISPLAY CODE HERE
                                        write-host $SessionStat.accessPointStatusAndStats.date "|" $SessionStatsVM.UAGname "| Open:" $SessionStat.accessPointStatusAndStats.openIncomingConnectionCount "| Count:" $SessionStat.accessPointStatusAndStats.sessionCount "| Auth'd:" $SessionStat.accessPointStatusAndStats.authenticatedSessionCount "| Auth'd HZ:" $SessionStat.accessPointStatusAndStats.authenticatedViewSessionCount "|" $SessionStat.accessPointStatusAndStats.viewEdgeServiceStats.protocol[0].name $SessionStat.accessPointStatusAndStats.viewEdgeServiceStats.protocol[0].sessions "|" $SessionStat.accessPointStatusAndStats.viewEdgeServiceStats.protocol[1].name $SessionStat.accessPointStatusAndStats.viewEdgeServiceStats.protocol[1].sessions "|" $SessionStat.accessPointStatusAndStats.viewEdgeServiceStats.protocol[2].name $SessionStat.accessPointStatusAndStats.viewEdgeServiceStats.protocol[2].sessions "| Success Logins:" $SessionStat.accessPointStatusAndStats.authentication.successLogins "Failed Logins:" $SessionStat.accessPointStatusAndStats.authentication.failedLogins "| Overall:" $SessionStat.accessPointStatusAndStats.overAllStatus.status "| Auth Broker:" $SessionStat.accessPointStatusAndStats.authentication.authBrokerStatus.status "| HZ Backend:" $SessionStat.accessPointStatusAndStats.viewEdgeServiceStats.backendStatus.Status "| HZ Frontend:" $SessionStat.accessPointStatusAndStats.viewEdgeServiceStats.edgeServiceStatus.Status
                                    } catch {
                                        Write-Host "Encountered exception running Put UAG User SessionStatificate Method:" $Error[0].Exception.Message "for UAG:" $SessionStatsVM.UAGname -ForegroundColor Red
                                        $key = @()
                                        $SecurePassword = @()
                                        $BSTR = @()
                                        $UserUnsecurePassword = @()
                                ...
                                write-host "WARNING! - PRESS '1' TO STOP THE SESSION STATS DISPLAY AS IT WILL CONTINUOUSLY LOOP"
                                write-host "----------"
                                Start-Sleep -Seconds 60 #60 seconds is the UAG sample rate so no point in going faster
                                if ([Console]::KeyAvailable) {
                                    # read the key, and consume it so it won't
                                    # be echoed to the console:
                                    $keyInfo = [Console]::ReadKey($true)
                                    if ($keyInfo.KeyChar -eq "1") {
                                        $loop = $false
                                    } else {}
                                }
                            } #END OF WHILE LOOP
                            ...
                            $Userkeyfoo = @()
                            $UserSessionStatfoo = @()
                            $SetSessionStatsUserUrl = @()
                            $SessionStat = @()
                            $keyInfo = @()
                            #END OF MENU-DAY2OPS-SESSIONSTATS-DISPLAY
                        } elseif ($SessionStatsOption -eq "2") { #MENU-DAY2OPS-SESSIONSTATS-DONE
                            #Exit Update Mode
                            $SessionStatsMode = $false
                            #END OF MENU-DAY2OPS-SESSIONSTATS-DONE
                        } else {
                            write-host "ERROR: Invalid selection entered, please try again"
                        }
                    } #END OF SESSIONSTATS MODE MENU LOOP
                } else {
                    #Could not find any records for UAGs with that Horizon Pod name supplied
                    write-host "ERROR: Could not find any UAG's for Horizon Pod " $HorizonPod "in 'master-uag-list.csv'"
                }
                #END OF MENU-DAY2OPS-SESSIONSTATS-DONE
            } elseif ($UpdateOption -eq "7") { #MENU-DAY2OPS-DONE
